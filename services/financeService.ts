...
export const calculateValue = (name: string, stock: StockConfig, price: number): CalculatedStock => {
  ...
  // 目标价格pe计算,2 种方式 pe 都封顶30
  let normalPe = (Math.min(stock.增速pe || 0, 30) + Math.min(30, (stock.目标价格 || 0) / stock.动态收益))/2
  ...
  const v1Num = calculatePev(price) + calculatePbv(price)
  const v1 = v1Num.toFixed(2);

  const p2 = price * 1.05;
  const v2 = (calculatePev(p2) + calculatePbv(p2)).toFixed(2);

  const p3 = price * 0.95;
  const v3 = (calculatePev(p3) + calculatePbv(p3)).toFixed(2);
